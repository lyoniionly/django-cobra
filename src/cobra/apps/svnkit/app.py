...
class SvnkitApplication(Application):
    ...
    svn_jump_revision_view = get_class('svnkit.views', 'SvnJumpRevisionView')
    ...
    def get_urls(self):
        urls = [
            ...
            url(r'^(?P<organization_slug>[\w_-]+)/(?P<project_id>[\w_-]+)/jump/$', self.svn_jump_revision_view.as_view(),
                name='jump-revision'),
