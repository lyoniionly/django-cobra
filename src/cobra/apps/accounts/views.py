...
from cobra.apps.accounts.utils import get_password_reset_url
...
RegisterUserMixin = get_class(
    'accounts.mixins', 'RegisterUserMixin')
Dispatcher = get_class('accounts.utils', 'Dispatcher')

PasswordChangeForm = get_class('accounts.forms', 'PasswordChangeForm')
ProfileForm, ConfirmPasswordForm = get_classes(
    'accounts.forms', ['ProfileForm', 'ConfirmPasswordForm'])
Email = get_model('accounts', 'Email')
...
class AccountRegistrationView(RegisterUserMixin, generic.FormView):
    ...
    def get_logged_in_redirect(self):
        return reverse('accounts:summary')
...
class LogoutView(generic.RedirectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for cookie in settings.COBRA_COOKIES_DELETE_ON_LOGOUT:
...
class ProfileDeleteView(PageTitleMixin, generic.FormView):
    ...
    template_name = 'accounts/profile/profile_delete.html'
    page_title = _('Delete profile')
    active_tab = 'profile'
    success_url = settings.COBRA_HOMEPAGE
...
class ChangePasswordView(PageTitleMixin, generic.FormView):
    ...
    template_name = 'accounts/profile/change_password_form.html'
...
class EmailHistoryView(PageTitleMixin, generic.ListView):
    ...
    template_name = 'accounts/email/email_list.html'
    paginate_by = 20
...
class EmailDetailView(PageTitleMixin, generic.DetailView):
    """User email"""
    template_name = "accounts/email/email_detail.html"
