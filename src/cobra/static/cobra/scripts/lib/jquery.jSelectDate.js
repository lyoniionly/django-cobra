...
var jSelectDate = {
  ...
  init: function (els) {
    ...
    els.each(function () {
      ...
      selYear.required = 'required';
      selYear.dataset.error = "请选择年份";
      ...
      selMonth.required = 'required';
      selMonth.dataset.error = "请选择月份";
      ...
      selDay.required = 'required';
      selDay.dataset.error = "请选择日期";
  ...
  progressDaySize: function (el, isMonth) {
    if (isMonth == true) {
      ...
      else if (!jSelectDate.isLargeMonth(month)) {
    ...
    }); //add by lyon to refresh select2 widget
  }
...
jQuery.fn.jSelectDate = function (settings) {
  ...
  $('select.' + jSelectDate.settings.css).select2(options);
