...
SMTP_HOSTNAME = getattr(settings, 'COBRA_SMTP_HOSTNAME', 'localhost')
ENABLE_EMAIL_REPLIES = getattr(settings, 'COBRA_ENABLE_EMAIL_REPLIES', False)
...
class MessageBuilder(object):
    ...
    def build(self, to, reply_to=()):
        ...
        if ENABLE_EMAIL_REPLIES and 'X-Cobra-Reply-To' in headers:
            reply_to = headers['X-Cobra-Reply-To']
        else:
    ...
    def send_async(self, to=None):
        from cobra.tasks.email import send_email
        messages = self.get_built_messages(to)
